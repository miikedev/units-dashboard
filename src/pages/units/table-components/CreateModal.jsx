...
import {
    districtAtom,
    fetchedPositionsAtom,
    levelDetailsAtom,
    selectedCreateDistrictKeyAtom,
    selectedCreatedTownshipKeyAtom,
    selectedCreateStateKeyAtom,
    stateAtom,
    townshipAtom,
} from "../atoms";
...
const CreateModal = ({ isOpen, onClose }) => {
    ...
    const [selectedStateKey, setSelectedStateKey] = useAtom(
        selectedCreateStateKeyAtom,
    );
    const [selectedDistrictKey, setSelectedDistrictKey] = useAtom(
        selectedCreateDistrictKeyAtom,
    );
    const [selectedTownshipKey, setSelectedTownshipKey] = useAtom(
        selectedCreatedTownshipKeyAtom,
    );
    ...
    const onDistrictSelectionChange = (district) => {
        setSelectedDistrictKey(
            district
                ...
                : { district: null, level: null },
        );
        setLevelDetails({ district: district.name });
    };

    const onTownshipSelectionChange = (township) => {
        setSelectedTownshipKey(
            township
                ? { township, level: "township" }
                : { township: null, level: null },
        );
    ...
    const filteredPositions = fetchedPositions.filter(
        (position) => position.level === getCurrentLevel(),
    );
    ...
    const handleSubmit = (e) => {
        ...
        const selectedDistrict = disctricts?.find(
            (d) => d.key === formData?.district,
        );
        const selectedTownship = townships?.find(
            (t) => t.key === formData?.township,
        );
        const { _id: selectedPositionId } = fetchedPositions?.find(
            (p) => p?.name === formData?.position_id,
        );
        ...
        if (
            ...
            !selectedDistrictKey.district &&
            !selectedTownshipKey.township
        ) {
            setValidation({
                type: "location",
                msg: "Please select at least one location",
            });
            return; // ← Add this
        }
                ...
                <ModalContent>
                    {(onClose) => (
                        <>
                            ...
                            <ModalBody className="flex justify-center w-full bg-slate-100">
